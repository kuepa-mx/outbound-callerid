...
export default class OutboundCalleridPlugin extends FlexPlugin {
	...
	async init(flex, manager) {
		...
		if (
			manager.workerClient.attributes.lastSelectedCallerId &&
			callerIds.find(
				(o) =>
					o.phoneNumber === manager.workerClient.attributes.lastSelectedCallerId
			)
		) {
			console.log(
				PLUGIN_NAME,
				'Found a previous caller id: ',
				manager.workerClient.attributes.lastSelectedCallerId
			)
			manager.store.dispatch(
				NumberSelectActions.updateNumber(
					manager.workerClient.attributes.lastSelectedCallerId
				)
			)
		} else {
			manager.store.dispatch(
				NumberSelectActions.updateNumber(callerIds[0].phoneNumber)
			)
		}
